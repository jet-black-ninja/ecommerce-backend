...
interface ProductDocument extends Document {
  ...
  const getFilteredProducts = async (req: Request, res: Response): Promise<void> => {
    try {
      const { category = '', brand = '', sortBy = 'price-lowtohigh' } = req.query as {
        category?: string;
        brand?: string;
        sortBy?: SortOption;
      };
      ...
      const products: ProductDocument[] = await Product.find(filters).sort(sort);
